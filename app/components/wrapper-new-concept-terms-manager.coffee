...
WrapperNewConceptTermsManagerComponent = Ember.Component.extend
  tagName: ''
  ...
  altTerms: []
  hiddenTerms: []
  mergedArrays: Ember.computed.union 'prefTerms', 'altTerms', 'hiddenTerms'
  newTerms: Ember.computed 'newPrefTerm', 'newAltTerm', 'newHiddenTerm', ->
    [@get('newPrefTerm'), @get('newAltTerm'), @get('newHiddenTerm')]
  allTerms: Ember.computed.union 'mergedArrays', 'newTerms'

  targetLanguage: "en"
  modelOrLanguageChangeChecker: Ember.observer('targetLanguage', ->
# If those properties are not set, no point in going further
    unless @get('targetLanguage') then return
    ...
    promises.push Ember.RSVP.Promise.all(prom).then =>
      Ember.RSVP.hash(
        pref: @get('model.prefLabels')
        alt: @get('model.altLabels')
        hidden: @get('model.hiddenLabels')
      ).then (hash) =>
        pref = hash['pref']
        alt = hash['alt']
        hidden = hash['hidden']
...
# terms that will be passed as parameters
  ...
  saveTerm: (list, addToList, term, name, index) ->
    ...
    if term.get('isDeleted')
      list.removeObject(term)
      @sendAction('removeTerm', term)
    else
      if addToList
        list.addObject(term)
        @sendAction('addTerm', term)
...
`export default WrapperNewConceptTermsManagerComponent`
